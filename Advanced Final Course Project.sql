...
-- Advanced MySQL Final Course Project
...
*/
SELECT
	YEAR(website_sessions.website_session_id) AS yr,
    QUARTER(website_sessions.website_session_id) AS qtr,
...
Could you pull a quarterly view of orders from Gsearch nonbrand, 
...
/*
4. Next, let’s show the overall session to order conversion rate trends 
for those same channels, by quarter. Please also make a note of 
...
SELECT
    ...
    SUM(CASE WHEN product_id = 3 THEN price_usd - cogs_usd ELSE NULL END) birthdaybear_marg,
    SUM(CASE WHEN product_id = 4 THEN price_usd ELSE NULL END) AS minibear_rev,
    SUM(CASE WHEN product_id = 4 THEN price_usd - cogs_usd ELSE NULL END) AS minibear_rev,
...
SELECT
    ...
    COUNT(DISTINCT products_pageviews.website_session_id) AS sessions_products_page,
    COUNT(DISTINCT website_pageviews.website_session_id) AS clicked_to_next_page,
    COUNT(DISTINCT website_pageviews.website_session_id)/COUNT(DISTINCT products_pageviews.website_session_id) AS clicthrough_rt,
    COUNT(DISTINCT orders.order_id) AS orders,
...
Based on all the analysis you’ve done, could you share some recommendations and
opportunities for us going forward? 
No right or wrong answer here I’d just like to hear your perspective!
*/

/*
Keep an eye on areas of the website that shows drop off.
Continue to look into products that cross sold really well.
Discuss why certain products cross sold better than the rest.
Find relationship between page traffic and sales.
/*